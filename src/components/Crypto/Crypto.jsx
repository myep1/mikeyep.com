...
const Crypto = () => {
  ...
  const onKeyReady = useCallback(({ key, salt, iv }) => {
    setKey(key);
    setSalt(salt);
    setIv(iv);
  }, []);
  ...
  const doEncrypt = async () => {
    setError("");
    if (!key || !salt || !iv) return setError("Missing key, salt, or IV");
    try {
  ...
  return (
    <div style={{ padding: "1rem" }}>
      <h2>AES-GCM + PBKDF2 (modular)</h2>

      <AES256GCM onKeyReady={onKeyReady}>
        {({ key, salt, iv }) => (
          <>
            <textarea
